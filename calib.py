...
def save_frames_single_camera(camera_name):
    ...
    print(f"\nStarting frame capture for {camera_name}.")
    print("Press SPACE in this terminal to start, or Q to quit.")
    ...
    try:
        ...
        while True:
            ...
            if not start:
                cv.putText(frame_small, "Press SPACEBAR in the terminal to start", (50,50), cv.FONT_HERSHEY_COMPLEX, 1, (0,0,255), 1)
            ...
            if cv.waitKey(1) & 0xFF == 27: break # Allow exit with ESC from OpenCV window
...
def save_frames_two_cams(camera0_name, camera1_name):
    ...
    print(f"\nStarting stereo frame capture for {camera0_name} and {camera1_name}.")
    print("Press SPACE in this terminal to start, or Q to quit.")
